...
import KeybindBranches exposing (generateAlbumKeybindings, generateBranches, createBranchDictionary)
...
branchGenerationTests =
    describe "Branch Generation Tests"
        [ test "Single word 'Tech' generates ['tech']" <|
            \_ ->
                let
                    branches = generateBranches "Tech"
                in
        ...
        , test "Multi-word 'The World' generates ['tworld', 'thworld', 'theworld']" <|
            \_ ->
                let
                    branches = generateBranches "The World"
                in
        ...
        , test "Multi-word 'To Be Sorted' generates at least 2 branches including first and last" <|
            \_ ->
                let
                    branches = generateBranches "To Be Sorted"
                in
                Expect.all
        ...
        , test "Long single word 'Christianity' generates ['christianity'] in Stage 1" <|
            \_ ->
                let
                    branches = generateBranches "Christianity"
                in
        ...
        , test "Four letter word 'Cool' generates ['cool']" <|
            \_ ->
                let
                    branches = generateBranches "Cool"
                in
        ...
        [ test "Conflicting branches are removed entirely" <|
            \_ ->
                let
                    ...
                    branchDict = createBranchDictionary albums
                    
                    -- Both Christianity and Christy would generate "chry", 
                    -- so neither should get it
                    hasChry = Dict.member "chry" branchDict
                in
        ...
        , test "Non-conflicting branches are preserved" <|
            \_ ->
                let
                    ...
                    hasTech = Dict.member "tech" branchDict
                    hasGeneral = Dict.member "general" branchDict
                in
                Expect.all
        ...
        [ test "'Tech' vs 'Technology' creates prefix conflict" <|
            \_ ->
                let
                    ...
                    finalKeybindings = generateAlbumKeybindings albums
                    
                    techBinding = Dict.get "1" finalKeybindings
                    technologyBinding = Dict.get "2" finalKeybindings
                in
...
iterativeShorteningTests =
    describe "Iterative Shortening Tests"
        [ test "Debug branch dictionary creation for Comics/Communism/Comedians" <|
            \_ ->
                let
                    ...
                    branchDict = createBranchDictionary albums
                    keys = Dict.keys branchDict |> List.sort
                in
        ...
        , test "Debug full pipeline for Comics/Communism/Comedians" <|
            \_ ->
                let
                    ...
                    comicsResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                    communismResult = Dict.get "2" finalKeybindings |> Maybe.withDefault "MISSING"  
                    comediansResult = Dict.get "3" finalKeybindings |> Maybe.withDefault "MISSING"
                in
        ...
        , test "Debug Apple/Banana shortening" <|
            \_ ->
                let
                    ...
                    appleResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                    bananaResult = Dict.get "2" finalKeybindings |> Maybe.withDefault "MISSING"
                in
        ...
        , test "Based/Based Test/Boss Took should get [ba, bte, bto]" <|
            \_ ->
                let
                    ...
                    basedResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                    basedTestResult = Dict.get "2" finalKeybindings |> Maybe.withDefault "MISSING"
                    bossTookResult = Dict.get "3" finalKeybindings |> Maybe.withDefault "MISSING"
                in
                let
                    debugInfo = "Actual: Based=" ++ basedResult ++ ", BasedTest=" ++ basedTestResult ++ ", BossTook=" ++ bossTookResult
                in
                Expect.all
        ...
        , test "Based alone should get b (optimal)" <|
            \_ ->
                let
                    albums = [ createTestAlbum "1" "Based" ]
                    finalKeybindings = generateAlbumKeybindings albums
                    basedResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                in
        ...
        , test "Ukraine/Unknown/Unexpected should get [uk, unk, une]" <|
            \_ ->
                let
                    ...
                    ukraineResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                    unknownResult = Dict.get "2" finalKeybindings |> Maybe.withDefault "MISSING"
                    unexpectedResult = Dict.get "3" finalKeybindings |> Maybe.withDefault "MISSING"
                in
                -- Debug what we actually get
                let
                    debug = "Ukraine=" ++ ukraineResult ++ ", Unknown=" ++ unknownResult ++ ", Unexpected=" ++ unexpectedResult
                in
                -- Ukraine should get "uk", not "u" - conflict detection should prevent over-shortening
                Expect.all
        ...
        , test "Non-alphanumeric characters should be filtered out" <|
            \_ ->
                let
                    ...
                    mediaResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                in
                -- Debug what branches are generated for "Media - LotR"
                let
                    branches = generateBranches "Media - LotR"
                    branchesDebug = "Branches: " ++ String.join ", " branches
                in
                -- Should filter out "-" and produce branches like "mlotr", then shorten optimally  
                -- "m" is actually the optimal result when no conflicts exist
                Expect.all
        ...
        , test "Single character album with conflicts shortens appropriately" <|
            \_ ->
                let
                    ...
                    jResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                    jazzResult = Dict.get "2" finalKeybindings |> Maybe.withDefault "MISSING"
                in
        ...
        , test "Multiple single characters with other albums" <|
            \_ ->
                let
                    ...
                    jResult = Dict.get "1" finalKeybindings |> Maybe.withDefault "MISSING"
                    kResult = Dict.get "2" finalKeybindings |> Maybe.withDefault "MISSING"
                    jazzResult = Dict.get "3" finalKeybindings |> Maybe.withDefault "MISSING"
                    kitchenResult = Dict.get "4" finalKeybindings |> Maybe.withDefault "MISSING"
                in
        ...
        , test "Iterative shortening removes longest strings first" <|
            \_ ->
                let
                    ...
                    comicsBinding = Dict.get "1" finalKeybindings
                    communismBinding = Dict.get "2" finalKeybindings
                    comediansBinding = Dict.get "3" finalKeybindings
                in
                case (comicsBinding, communismBinding, comediansBinding) of
                    (Just comics, Just communism, Just comedians) ->
                        ...
                        let
                            allBindings = [comics, communism, comedians]
                            allUnique = List.length allBindings == List.length (removeDuplicates allBindings)
                            noPrefixConflicts = not (hasPrefixConflicts allBindings)
                        in
        ...
        , test "Basic shortening works without conflicts" <|
            \_ ->
                let
                    ...
                    appleBinding = Dict.get "1" finalKeybindings
                    bananaBinding = Dict.get "2" finalKeybindings
                    cherryBinding = Dict.get "3" finalKeybindings

                    allBindings = [appleBinding, bananaBinding, cherryBinding]
                    allPresent = List.all (\binding -> binding /= Nothing) allBindings
                in
                if allPresent then
                    let
                        actualBindings = List.filterMap identity allBindings
                        allUnique = List.length actualBindings == List.length (removeDuplicates actualBindings)
                        noPrefixConflicts = not (hasPrefixConflicts actualBindings)
                    in
                    Expect.all
        ...
        [ test "Complex real-world album set has no conflicts" <|
            \_ ->
                let
                    ...
                    finalKeybindings = generateAlbumKeybindings albums
                    allKeybindings = Dict.values finalKeybindings
                    
                    -- Check no duplicates
                    noDuplicates = List.length allKeybindings == List.length (removeDuplicates allKeybindings)
                    
                    -- Check no prefix conflicts
                    noPrefixConflicts = not (hasPrefixConflicts allKeybindings)
                    
                    -- Check all albums got keybindings
                    allAlbumsHaveKeybindings = List.length allKeybindings == List.length albums
                in
                Expect.all
...
removeDuplicates list =
    list
        |> List.foldl (\item acc -> if List.member item acc then acc else item :: acc) []
        |> List.reverse
