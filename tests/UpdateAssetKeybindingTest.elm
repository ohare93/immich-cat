...
testAsset = createTestAssetWithActions "test-id" "Test Asset"
        ...
        [ test "valid keybinding character starts keybinding mode" <|
            \_ ->
                let
                    ...
                    result = handleEditAssetInput "g" NormalMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    UpdateAssetSearch newSearch ->
                        Expect.equal "g" newSearch.partialKeybinding
                    _ -> Expect.fail "Expected valid keybinding character to start keybinding mode"

        , test "exact keybinding match toggles album membership" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" "g"
                    result = handleEditAssetInput "e" KeybindingMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    ToggleAlbumMembership album ->
                        Expect.equal "General" album.albumName
                    _ -> Expect.fail "Expected exact keybinding match to toggle album membership"

        , test "partial keybinding accumulation works correctly" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" "p"
                    result = handleEditAssetInput "h" KeybindingMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    UpdateAssetSearch newSearch ->
    ...
    describe "Invalid Character Rejection Tests"
        [ test "invalid keybinding character is rejected from NormalMode" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" ""
                    result = handleEditAssetInput "z" NormalMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    InvalidKeybindingInput invalidChar _ ->
                        Expect.equal "z" invalidChar
                    _ -> Expect.fail "Expected invalid keybinding character to be rejected"

        , test "invalid keybinding character is rejected in KeybindingMode" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" "g"
                    result = handleEditAssetInput "z" KeybindingMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    InvalidKeybindingInput invalidChar _ ->
                        Expect.equal "z" invalidChar
                    _ -> Expect.fail "Expected invalid keybinding character to be rejected in KeybindingMode"
        ...
        , test "invalid character rejection preserves partial keybinding" <|
            \_ ->
                ...
                case result of
                    InvalidKeybindingInput invalidChar _ ->
                        Expect.equal "z" invalidChar
                    _ -> Expect.fail "Expected invalid character to be rejected while preserving partial keybinding"
        ]
    ...
    describe "Text Search Fallback Tests"
        [ test "uppercase letters start text search in InsertMode" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" ""
                    result = handleEditAssetInput "X" InsertMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    UpdateAssetSearch newSearch ->
                        Expect.equal "X" newSearch.searchString
                    _ -> Expect.fail "Expected uppercase letter to start text search"

        , test "space characters work for text search in InsertMode" <|
            \_ ->
                let
                    search = createTestAlbumSearch "test" ""
                    result = handleEditAssetInput " " InsertMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    UpdateAssetSearch newSearch ->
                        Expect.equal "test " newSearch.searchString
                    _ -> Expect.fail "Expected space to continue text search"

        , test "empty keybindings dictionary allows fallback" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" ""
                    result = handleEditAssetInput "g" NormalMode testAsset search Dict.empty testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    NoAssetAction ->
                        Expect.pass  -- This is the expected behavior when no keybindings exist
                    _ -> Expect.fail "Expected no action when no keybindings exist"
        ]
...
consistencyWithAlbumBrowseTests =
    describe "Consistency with Album Browse Mode Tests"
        [ test "ViewAssets keybinding validation matches album browse behavior" <|
            \_ ->
                ...
                case result of
                    InvalidKeybindingInput invalidChar _ ->
                        Expect.equal "z" invalidChar
                    _ -> Expect.fail "Expected ViewAssets to reject invalid keybinding just like album browse"
        ...
        , test "ViewAssets exact match behavior matches album browse" <|
            \_ ->
                let
                    ...
                    result1 = handleEditAssetInput "g" NormalMode testAsset search testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                ...
                case result2 of
                    ToggleAlbumMembership album ->
                        Expect.equal "General" album.albumName
                    _ -> Expect.fail "Expected ViewAssets exact match to work like album browse"
        ...
        , test "warning state is properly handled in ViewAssets mode" <|
            \_ ->
                ...
                case result of
                    InvalidKeybindingInput invalidChar _ ->
                        Expect.equal "z" invalidChar
                    _ -> Expect.fail "Expected warning state to be handled in ViewAssets mode"
        ...
        , test "invalid warning clears when entering valid keybinding character in ViewAssets" <|
            \_ ->
                let
                    ...
                    result = handleEditAssetInput "h" KeybindingMode testAsset searchWithWarning testAlbumKeybindings testKnownAlbums 800 testCurrentAssets
                in
                case result of
                    UpdateAssetSearch newSearch ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Expected valid character to update search and clear warning in ViewAssets"
        ...
        , test "ViewAssets user scenario: 'azb' sequence with 'ab' keybinding should work correctly" <|
            \_ ->
                let
                    ...
                    testKnownAlbumsCustom = Dict.fromList [("album1", createTestAlbum "album1" "Test Album")]
                    
                    -- Start fresh and type "a"
                    search1 = createTestAlbumSearch "" ""
                    result1 = handleEditAssetInput "a" NormalMode testAsset search1 testKeybindingsCustom testKnownAlbumsCustom 800 testCurrentAssets
                    
                    -- Should get partial keybinding "a"
                    searchAfterA = case result1 of
                        UpdateAssetSearch newSearch -> newSearch
                        _ -> search1
                    
                    -- Type "z" (invalid) - should be rejected 
                    result2 = handleEditAssetInput "z" KeybindingMode testAsset searchAfterA testKeybindingsCustom testKnownAlbumsCustom 800 testCurrentAssets
                    
                    -- Type "b" after the rejected "z" - should complete "ab" keybinding
                    result3 = handleEditAssetInput "b" KeybindingMode testAsset searchAfterA testKeybindingsCustom testKnownAlbumsCustom 800 testCurrentAssets
                in
        ...
        , test "EXACT USER ISSUE: correct->incorrect->correct should not show warning" <|
            \_ ->
                let
                    testKeybindingsCustom = Dict.fromList [("album1", "ac")]
                    testKnownAlbumsCustom = Dict.fromList [("album1", createTestAlbum "album1" "Test Album")]
                    ...
                    result2 = handleEditAssetInput "b" KeybindingMode testAsset searchAfterA testKeybindingsCustom testKnownAlbumsCustom 800 testCurrentAssets
                    
                    -- Step 3: Type correct key "c" - should clear warning and complete keybinding
                    result3 = handleEditAssetInput "c" KeybindingMode testAsset searchAfterA testKeybindingsCustom testKnownAlbumsCustom 800 testCurrentAssets
                in
                case (result2, result3) of
