        ...
        , test "no-match scenario rejects invalid keybinding" <|
            \_ ->
                ...
                case result of
                    ...
                    _ -> Expect.fail "Expected invalid keybinding rejection"
        ...
        , test "Enter selects top match" <|
            \_ ->
                let
                    ...
                    search = { searchString = "gen", partialKeybinding = "", selectedIndex = 0, albumScores = Dict.fromList [("album1", 100)], pagination = { currentPage = 1, itemsPerPage = 20, totalItems = 1 }, invalidInputWarning = Nothing }
        ...
        , test "PageUp updates pagination" <|
            \_ ->
                let
                    ...
                    search = { searchString = "", partialKeybinding = "", selectedIndex = 0, albumScores = Dict.empty, pagination = { currentPage = 2, itemsPerPage = 20, totalItems = 100 }, invalidInputWarning = Nothing }
...
textSearchTests =
    describe "Text Search Tests"
        [ test "non-keybinding letters are rejected as invalid" <|
            \_ ->
                ...
                case result of
                    InvalidKeybindingInput invalidInput _ ->
                        Expect.equal "x" invalidInput
                    _ -> Expect.fail "Expected non-keybinding letter to be rejected"
        ...
        , test "helper functions work correctly for uppercase letters" <|
            \_ ->
                let
                    isKeybindingLetterResult = Helpers.isKeybindingLetter "X"
                    isSupportedSearchLetterResult = Helpers.isSupportedSearchLetter "X"
                in
                Expect.all
        ...
        , test "invalid warning clears when entering valid keybinding character" <|
            \_ ->
                let
                    ...
                    searchWithWarning = createTestAlbumSearchWithWarning "" "p" "Invalid character: z"
                    
                    -- Type a valid character "h" after "p" to make "ph" (partial keybinding for "pho")
                    result = handleAlbumBrowseInput "h" searchWithWarning testAlbumKeybindings testAlbums
                in
                case result of
                    UpdateAlbumSearch newSearch ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Expected valid character to update search and clear warning"
                    
        , test "user scenario: 'azb' sequence with 'ab' keybinding should work correctly" <|
            \_ ->
                let
                    -- Create custom test data with keybinding "ab"  
                    testKeybindingsCustom = Dict.fromList [("album1", "ab")]
                    testAlbumsCustom = Dict.fromList [("album1", createTestAlbum "album1" "Test Album")]
                    
                    -- Start fresh and type "a"
                    search1 = createTestAlbumSearch "" ""
                    result1 = handleAlbumBrowseInput "a" search1 testKeybindingsCustom testAlbumsCustom
                    
                    -- Should get partial keybinding "a"
                    searchAfterA = case result1 of
                        UpdateAlbumSearch newSearch -> newSearch
                        _ -> search1
                    
                    -- Type "z" (invalid) - should be rejected 
                    result2 = handleAlbumBrowseInput "z" searchAfterA testKeybindingsCustom testAlbumsCustom
                    
                    -- Type "b" after the rejected "z" - should complete "ab" keybinding
                    result3 = handleAlbumBrowseInput "b" searchAfterA testKeybindingsCustom testAlbumsCustom
                in
        ...
        , test "EXACT USER ISSUE: correct->incorrect->correct should not show warning" <|
            \_ ->
                let
                    ...
                    testKeybindingsCustom = Dict.fromList [("album1", "ac")] -- Use "ac" so "ab" doesn't complete
                    testAlbumsCustom = Dict.fromList [("album1", createTestAlbum "album1" "Test Album")]
                    ...
                    result2 = handleAlbumBrowseInput "b" searchAfterA testKeybindingsCustom testAlbumsCustom
                    -- Simulate what happens in UI: InvalidKeybindingInput creates warning
                    searchWithWarning = case result2 of
                    ...
                    result3 = handleAlbumBrowseInput "c" searchWithWarning testKeybindingsCustom testAlbumsCustom
                in
                case result3 of
                    SelectAlbumForView album ->
...
invalidInputTests =
    describe "Invalid Input Tests"
        [ test "invalid character after partial keybinding is rejected" <|
            \_ ->
                ...
                case result of
                    InvalidKeybindingInput invalidInput _ ->
                        Expect.equal "x" invalidInput
                    _ -> Expect.fail "Expected invalid input rejection"
        ...
        , test "valid character after partial keybinding is accepted" <|
            \_ ->
                let
                    search = createTestAlbumSearch "" "p"
                    -- Type "h" after "p" - should be valid since "pho" is a valid keybinding
                    result = handleAlbumBrowseInput "h" search testAlbumKeybindings testAlbums
                in
                case result of
                    UpdateAlbumSearch newSearch ->
                        Expect.equal "ph" newSearch.partialKeybinding
                    _ -> Expect.fail "Expected valid character acceptance"
        ...
        , test "exact match after partial keybinding selects album" <|
            \_ ->
                ...
                case result2 of
                    SelectAlbumForView album ->
                        Expect.equal "General" album.albumName
                    _ -> Expect.fail "Expected exact match to select album"
        ...
        , test "debug: specific keybinding validation scenario" <|
            \_ ->
                let
                    ...
                    result1 = handleAlbumBrowseInput "a" search testKeybindingsCustom testAlbumsCustom
                ...
                case result2 of
                    ...
                    _ -> Expect.fail "Expected 'z' to be rejected as invalid input"
        ...
        , test "debug: isValidNextCharacter function behavior" <|
            \_ ->
                let
                    ...
                    validNextAfterA = ViewAlbums.getNextAvailableCharacters "a" testKeybindingsCustom
                    isZValidAfterA = ViewAlbums.isValidNextCharacter "a" 'z' testKeybindingsCustom
                    isAValidAfterA = ViewAlbums.isValidNextCharacter "a" 'a' testKeybindingsCustom
                    isBValidAfterA = ViewAlbums.isValidNextCharacter "a" 'b' testKeybindingsCustom
                    isCValidAfterA = ViewAlbums.isValidNextCharacter "a" 'c' testKeybindingsCustom
                in
                Expect.all
        ...
        , test "explicit test: partialKeybinding should NOT be modified on invalid input" <|
            \_ ->
                let
                    ...
                    searchWithA = createTestAlbumSearch "" "a"
                    
                    -- Type 'z' - should be rejected
                    result = handleAlbumBrowseInput "z" searchWithA testKeybindingsCustom testAlbumsCustom
                in
                case result of
                    UpdateAlbumSearch newSearch ->
                    ...
                    _ -> Expect.fail "Unexpected result type"
        ...
        , test "reproduce user's exact issue: typing 'z' after no matches should reject" <|
            \_ ->
                let
                    ...
                    result = handleAlbumBrowseInput "z" search testAlbumKeybindings testAlbums
                in
                case result of
                    InvalidKeybindingInput invalidChar _ ->
                        Expect.equal "z" invalidChar
                    _ -> Expect.fail "Expected 'z' to be rejected as invalid keybinding"
        ...
        , test "INVESTIGATE: what happens with multiple invalid characters after valid partial" <|
            \_ ->
                let
                    ...
                    -- Type 'z' - should be rejected
                    result2 = handleAlbumBrowseInput "z" searchAfterA testKeybindingsCustom testAlbumsCustom
                    searchAfterZ = case result2 of
                        UpdateAlbumSearch newSearch -> newSearch
                        InvalidKeybindingInput _ _ -> searchAfterA  -- Should remain unchanged
                        _ -> searchAfterA
                    
                    -- Type another 'z' - should be rejected
                    result3 = handleAlbumBrowseInput "z" searchAfterZ testKeybindingsCustom testAlbumsCustom
                    searchAfterZZ = case result3 of
                        UpdateAlbumSearch newSearch -> newSearch
                        InvalidKeybindingInput _ _ -> searchAfterZ  -- Should remain unchanged
                        _ -> searchAfterZ
                        
                    -- Type 'a' - should work and make "aa"
                    result4 = handleAlbumBrowseInput "a" searchAfterZZ testKeybindingsCustom testAlbumsCustom
                in
                case result4 of
                    SelectAlbumForView album ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Expected typing 'a' after rejecting 'z' chars to select 'aa' album"
        ...
        , test "DIRECT CHECK: isValidNextCharacter with real scenario" <|
            \_ ->
                let
                    ...
                    isZValidFromEmpty = ViewAlbums.isValidNextCharacter "" 'z' testAlbumKeybindings
                    availableFromEmpty = ViewAlbums.getNextAvailableCharacters "" testAlbumKeybindings
                    
                    -- Check what happens when we type 'z' after 'g'
                    isZValidAfterG = ViewAlbums.isValidNextCharacter "g" 'z' testAlbumKeybindings
                    availableAfterG = ViewAlbums.getNextAvailableCharacters "g" testAlbumKeybindings
                in
                Expect.all
        ...
        , test "SIMULATE BUG: typing multiple invalid characters" <|
            \_ ->
                let
                    ...
                    result2 = handleAlbumBrowseInput "z" searchAfterA testKeybindingsCustom testAlbumsCustom
                    result3 = handleAlbumBrowseInput "z" searchAfterA testKeybindingsCustom testAlbumsCustom
                    result4 = handleAlbumBrowseInput "z" searchAfterA testKeybindingsCustom testAlbumsCustom
                in
                Expect.all
                    [ \_ -> case result2 of
                        InvalidKeybindingInput _ _ -> Expect.pass
                        _ -> Expect.fail "First 'z' should be rejected"
                    , \_ -> case result3 of
                        InvalidKeybindingInput _ _ -> Expect.pass
                        _ -> Expect.fail "Second 'z' should be rejected"
                    , \_ -> case result4 of
                        InvalidKeybindingInput _ _ -> Expect.pass
                        _ -> Expect.fail "Third 'z' should be rejected"
                    ]
                    ()
        ...
        , test "USER REQUIREMENT: invalid keybinding chars should be rejected, not fall back to text search" <|
            \_ ->
                let
                    ...
                    customKeybindings = Dict.fromList [("1", "aa"), ("2", "abc"), ("3", "ac")]
                    customAlbums = Dict.fromList [("1", createTestAlbum "1" "AA"), ("2", createTestAlbum "2" "ABC"), ("3", createTestAlbum "3" "AC")]
                    
                    search = createTestAlbumSearch "" ""
                    
                    -- Type 'a' - should work
                    result1 = handleAlbumBrowseInput "a" search customKeybindings customAlbums
                    searchAfterA = case result1 of
                        UpdateAlbumSearch newSearch -> newSearch
                        _ -> search
                    
                    -- Type 'z' - should be REJECTED, not fall back to text search
                    result2 = handleAlbumBrowseInput "z" searchAfterA customKeybindings customAlbums
                    
                    -- Type 'q' - should be REJECTED, not fall back to text search
                    result3 = handleAlbumBrowseInput "q" searchAfterA customKeybindings customAlbums
                    
                    -- Type 'x' - should be REJECTED, not fall back to text search
                    result4 = handleAlbumBrowseInput "x" searchAfterA customKeybindings customAlbums
                in
                Expect.all
                    [ \_ -> Expect.equal "a" searchAfterA.partialKeybinding
                    , \_ -> case result2 of
                        InvalidKeybindingInput char _ -> Expect.equal "z" char
                        _ -> Expect.fail "Expected 'z' to be rejected"
                    , \_ -> case result3 of
                        InvalidKeybindingInput char _ -> Expect.equal "q" char
                        _ -> Expect.fail "Expected 'q' to be rejected"
                    , \_ -> case result4 of
                        InvalidKeybindingInput char _ -> Expect.equal "x" char
                        _ -> Expect.fail "Expected 'x' to be rejected"
                    ]
                    ()
        ]