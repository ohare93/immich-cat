...
import ViewAlbums exposing (AlbumSearch, AssetWithActions, InputMode(..), PropertyChange(..), usefulColours)
...
testAlbumKeybindings = Dict.fromList [("album1", "ge"), ("album2", "tr")]
                ...
                case result2 of
                    StayInAssets (EditAsset mode asset search) ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Expected to stay in InsertMode EditAsset state"
        ...
        , test "partialKeybinding remains empty during InsertMode" <|
            \_ ->
                ...
                case result of
                    ...
                    _ -> Expect.fail "Expected to stay in EditAsset state"
        ...
        , test "mode stays as InsertMode throughout typing" <|
            \_ ->
                ...
                case result3 of
                    ...
                    _ -> Expect.fail "Expected to stay in InsertMode"
        ...
        , test "Escape transitions back to NormalMode" <|
            \_ ->
                ...
                case result of
                    ...
                    _ -> Expect.fail "Expected to stay in NormalMode EditAsset state"
        ]
            ...
            \_ ->
                ...
                case result of
                    ...
                    AssetToggleAlbumMembership _ ->
                        ...
                        Expect.pass
                    _ -> Expect.fail "Expected to stay in KeybindingMode or toggle album"
                ...
                case result of
                    StayInAssets (EditAsset mode _ search) ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Expected to return to NormalMode"
        ]
        ...
        [ test "I from NormalMode transitions to InsertMode" <|
            \_ ->
                ...
                case result of
                    StayInAssets (EditAsset mode _ _) ->
                        Expect.equal InsertMode mode
                    _ -> Expect.fail "Expected to transition to InsertMode"
            ...
            \_ ->
                ...
                case result of
                    ...
                    AssetToggleAlbumMembership _ ->
                        ...
                        Expect.pass
                    _ -> Expect.fail "Expected to transition to KeybindingMode or toggle album"
                ...
                case (insertResult, keybindingResult) of
                    (StayInAssets (EditAsset mode1 _ _), StayInAssets (EditAsset mode2 _ _)) ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Expected both to return to NormalMode"
        ]
        ...
        [ test "empty strings in both fields result in NormalMode" <|
            \_ ->
                ...
                case result of
                    StayInAssets (EditAsset mode _ _) ->
                        Expect.equal NormalMode mode
                    _ -> Expect.fail "Expected NormalMode with empty strings"

        , test "mode preservation during legitimate UpdateAssetSearch" <|
            \_ ->
                let
                    initialSearch = createTestAlbumSearch "test" ""
                    initialState = EditAsset InsertMode testAsset initialSearch
                ...
                case result of
                    StayInAssets (EditAsset mode _ _) ->
                        Expect.equal InsertMode mode
                    _ -> Expect.fail "Expected to preserve InsertMode during navigation"
        ]
                ...
                case result of
                    StayInAssets (EditAsset mode _ search) ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "First letter should work in InsertMode"
                ...
                case result of
                    StayInAssets (EditAsset mode _ search) ->
                        Expect.all
                            ...
                            ()
                    _ -> Expect.fail "Second letter should continue search string in InsertMode"
        ...
        , test "keybinding functionality still works in NormalMode" <|
            \_ ->
                ...
                case result2 of
                    ...
                    _ -> Expect.fail "Keybinding 'ge' should trigger album toggle"
        ]