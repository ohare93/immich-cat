...
import Task
import Theme
import Types exposing (AssetSource(..), DeviceClass, Flags, ImageIndex, Model, Msg(..), NavigationHistoryEntry, PaginationState, SourceLoadState, Theme, UserMode(..))
            ...
            if key == "Escape" then
                ...
                case effectiveKey of
                    ...
                    _ ->
                        case model.userMode of
                            MainMenu menuState ->
                                if effectiveKey == "T" && not (View.isInInputMode model.userMode) then
