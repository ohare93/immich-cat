...
generateAlbumKeybindings albums =
    let
        albumCandidates =
            List.map (\album -> ( album.id, generateSimpleCandidates album.albumName )) albums

        -- Smart sorting: avoid single letters when they would block longer meaningful names
        sortedAlbumCandidates =
            smartSortAlbums albumCandidates

        resolvedKeybindings =
            assignKeybindingsInOrder sortedAlbumCandidates

        -- Post-process to upgrade keybindings when safe
        optimizedKeybindings =
            optimizeKeybindingsToShorter albums resolvedKeybindings
    in
...
smartSortAlbums albumCandidates =
    let
        ...
        ( blockingAlbums, nonBlockingAlbums ) =
            List.partition wouldBlockOthers albumCandidates
                ...
                ( albumId, candidates ) :: rest ->
                    case findFirstAvailableNonPrefix candidates used of
                        Just keybinding ->
                            let
                                newUsed =
                                    Dict.insert keybinding True used
                            in
                            ( albumId, keybinding ) :: assignHelper rest newUsed

                        Nothing ->
                            let
                                fallback =
                                    String.left 3 albumId

                                newUsed =
                                    Dict.insert fallback True used
                            in
                            ( albumId, fallback ) :: assignHelper rest newUsed
    in
...
generateSimpleCandidates albumName =
    let
        ...
        words =
            String.split " " cleanName |> List.filter (not << String.isEmpty)

        firstChar =
            String.left 1 cleanName

        firstTwoChars =
            String.left 2 cleanName

        firstThreeChars =
            String.left 3 cleanName

        wordAbbrev =
            words |> List.map (String.left 1) |> String.join ""
        ...
        allCandidates =
            if String.length cleanName <= 2 then
                ...
                [ firstTwoChars ]
                    ++ -- "general" -> "ge" gets priority
                       twoCharVariants
                    ++ [ firstThreeChars ]
                    ++ [ String.left 4 cleanName ]
                    ++ [ firstChar ]
                    ++ -- Single letter as backup
                       [ wordAbbrev ]
                -- word abbreviation (same as firstChar for single words)
                ...
                let
                    ...
                    sortedWordAbbrevs =
                        if String.length wordAbbrev <= 2 then
                            [ wordAbbrev ]
                            -- Good short abbreviation like "tt"
...
generateTwoCharVariants cleanName =
    let
        ...
        chars =
            String.toList cleanName |> List.drop 1 |> List.take 4

        variants =
            List.map (\c -> firstChar ++ String.fromChar c) chars
    in
    variants
...
-- Same quality, keep original order
...
optimizeKeybindingsToShorter albums keybindings =
    let
        albumDict =
            Dict.fromList (List.map (\album -> ( album.id, album.albumName )) albums)
                ...
                Just albumName ->
                    if String.length currentKb == 2 then
                        let
                            ...
                            isSimpleWord =
                                not (String.contains " " cleanAlbumName) && String.length cleanAlbumName >= 3

                            isOnlyAlbumWithThisLetter =
                                List.length albumsWithSameLetter == 1

                            isExactPrefix =
                                currentKb == String.left 2 cleanAlbumName
