...
import Element exposing (Element, alignTop, centerX, centerY, column, el, fill, fillPortion, height, html, paddingXY, px, row, spacing, text, width)
...
import Html.Events
import Immich exposing (ImmichAsset, ImmichAssetId, ImmichApiPaths)
...
calculateGridColumns screenWidth =
    let
        minItemWidth = 150
        maxColumns = 8
        calculatedColumns = screenWidth // minItemWidth
    in
...
getSelectedAssets : GridState -> List ImmichAssetId
getSelectedAssets state =
    Dict.keys state.selectedAssets

getSelectedAsset : GridState -> Maybe ImmichAssetId
getSelectedAsset state =
    case Dict.keys state.selectedAssets of
        [singleAsset] -> Just singleAsset
        _ -> state.focusedAssetId

moveSelection : String -> GridState -> List ImmichAsset -> GridState
moveSelection direction state assets =
    let
        assetIds = List.map .id assets
        currentIndex = 
            case state.focusedAssetId of
                Nothing -> 0
                Just assetId -> 
                    List.indexedMap (\i id -> if id == assetId then Just i else Nothing) assetIds
                        |> List.filterMap identity
                        |> List.head
                        |> Maybe.withDefault 0
        
        newIndex = case direction of
            "ArrowUp" -> max 0 (currentIndex - state.gridColumns)
            "ArrowDown" -> min (List.length assets - 1) (currentIndex + state.gridColumns)
            "ArrowLeft" -> max 0 (currentIndex - 1)
            "ArrowRight" -> min (List.length assets - 1) (currentIndex + 1)
            "Home" -> 0
            "End" -> List.length assets - 1
            _ -> currentIndex
...
selectAll : GridState -> List ImmichAsset -> GridState
selectAll state assets =
    { state | selectedAssets = Dict.fromList (List.map (\asset -> (asset.id, True)) assets) }

clearSelection : GridState -> GridState
...
handleGridKeyPress key state assets =
    case key of
        "ArrowUp" -> moveSelection "ArrowUp" state assets
        "ArrowDown" -> moveSelection "ArrowDown" state assets
        "ArrowLeft" -> moveSelection "ArrowLeft" state assets
        "ArrowRight" -> moveSelection "ArrowRight" state assets
        "Home" -> moveSelection "Home" state assets
        "End" -> moveSelection "End" state assets
        " " -> toggleSelection state
        _ -> state
...
viewGrid apiPaths apiKey state assets toMsg =
    let
        gridItems = List.indexedMap (createGridItem state) assets
        
        headerInfo = 
            let
                selectedCount = Dict.size state.selectedAssets
                totalCount = List.length assets
                modeText = if state.multiSelectMode then "Multi-select" else "Single-select"
            in
...
viewGridItems apiPaths apiKey state gridItems toMsg =
    let
        ...
        gridItemsHtml = List.map (viewGridItem apiPaths apiKey toMsg) gridItems
    in
...
viewGridItem apiPaths apiKey toMsg item =
    ...
    Html.div 
        ...
        , Html.div selectionIndicatorStyle 
            [ Html.text (if item.isSelected then "✓" else String.fromInt (item.index + 1))
            ]
        ]